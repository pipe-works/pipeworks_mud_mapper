...
class MapRoom(BaseModel):
    """A room in authoring format (includes coordinates).
    ...
    coordinates that help visualize the spatial layout.

    The key difference from Room is the ``coords`` field, which stores the
    room's position in 3D space. When exporting to a zone file, coordinates
    are stripped using ``to_room()``.
    ...
    Converting to game truth format::

        >>> room = map_room.to_room()
        >>> room.coords  # AttributeError - no coords in Room

    See Also
    --------
    Room : Game truth format (no coordinates).
    ...
    def to_room(self) -> Room:
        ...
        -------
        Room
            A Room instance without coordinates, suitable for zone export.
    ...
    def from_dict(cls, data: dict) -> "MapRoom":
        ...
        This method handles the legacy format where coords is stored as a list
        rather than a Coords object.

        Parameters
        ----------
        data : dict
            Room data dictionary, potentially with coords as [x, y, z] list.

        Returns
        -------
        MapRoom
            New MapRoom instance.
